...
from frontline.serializers import *
...
def AnnouncementList(request):
    ...
    if request.method =='GET':
        if request.user.role == "Admin":
            ...
            filterset = AnnouncementFilter(request.GET,queryset=Announcements.objects.filter(audiences=request.user.role))
        if not filterset.is_valid():
            return Response({"errors":filterset.errors},status=status.HTTP_400_BAD_REQUEST)
...
def SchoolProfileManager(request):
    try:
        school_profile = SchoolProfile.objects.get(user=request.user.id)
